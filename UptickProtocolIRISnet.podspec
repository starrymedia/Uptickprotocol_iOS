#
# Be sure to run `pod lib lint DmaIRIS_SDK_IOS.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'UptickProtocolIRISnet'
  s.version          = '0.1.1'
  s.summary          = 'A short description of UptickProtocolIRISnet.'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
TODO: Add long description of the pod here.
                       DESC

  s.homepage         = 'https://github.com/starrymedia/DmaIRIS_SDK_IOS'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'fan' => '447690182@qq.com' }
  s.source           = { :git => 'https://github.com/starrymedia/DmaIRIS_SDK_IOS.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '12.0'

  s.source_files = 'DmaIRIS_SDK_IOS/Classes/**/*'
  
  # s.resource_bundles = {
  #   'DmaIRIS_SDK_IOS' => ['DmaIRIS_SDK_IOS/Assets/*.png']
  # }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
  s.dependency 'Alamofire', '~> 5.2'
  s.dependency 'web3swift', '~>1.1.10'
  s.dependency 'HandyJSON', '5.0.0-beta'
  s.dependency 'gRPC-Swift', '1.0.0-alpha.20'

end
