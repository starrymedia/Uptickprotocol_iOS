...
public class SegwitAddrCoder {
    ...
    func convertBits(instart: Int = 0, from: Int, to: Int, pad: Bool, idata: Data) throws -> Data {
        var acc: Int = 0
        var bits: Int = 0
        let maxv: Int = (1 << to) - 1
        let maxAcc: Int = (1 << (from + to - 1)) - 1
        var odata = Data()
        for (index,_) in idata.enumerated() {
            let value = idata[index + instart] & 0xff;
            acc = ((acc << from) | Int(value)) & maxAcc
            bits += from
            while bits >= to {
                bits -= to
